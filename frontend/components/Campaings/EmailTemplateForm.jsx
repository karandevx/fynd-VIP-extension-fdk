...
const EmailTemplateForm = ({ setCurrentStep, setShowCreateCampaign, campaignId, companyId }) => {
  ...
  const handleRegenerate = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        throw new Error(response.data.message || 'Failed to generate email template');
      }
    } catch (error) {
  ...
  const buttonText = generationCount === 0 ? 'Generate and Preview' : 'Regenerate and Preview';
  ...
  return (
    <form className="space-y-6">
      ...
      {/* Email Prompt */}
      <div>
        <label htmlFor="emailPrompt" className="block text-sm font-medium text-gray-700 mb-1">Email Prompt</label>
        <textarea
          id="emailPrompt"
      ...
      <div className="flex justify-start mt-4">
        <button
          ...
          className={`px-4 py-2 bg-indigo-500 text-white rounded-md transition duration-150 ease-in-out shadow-sm text-sm font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 ${isButtonDisabled ? 'opacity-50 cursor-not-allowed' : 'hover:bg-indigo-600'}`}
      ...
      <div>
        ...
        <div className="border border-gray-200 rounded-md p-4 bg-gray-50 shadow-inner overflow-x-auto">
          {isGenerating ? (
            ...
            <div dangerouslySetInnerHTML={{ __html: generatedHtml }} className="prose max-w-none text-gray-800"></div>
          ) : (
            <p className="text-gray-500">Click 'Generate and Preview' to see the email content.</p>
          )}
        </div>
      </div>
      {/* Navigation Buttons */}
